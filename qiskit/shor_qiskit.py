...
class Shor:
    ...
    def phi_ADD(self, y: int):
        ...
        for i in range(n+1):  
            ...
            angles = [2*np.pi/2**(k+1) if bit==1 else 0 for k,bit in enumerate(bitstring[i:])]
    ...
    def circuit(self) -> QuantumCircuit:
        ...
        for i in range(t):
            A = a**(2**i) % N
            u_A = self.Uy(A)
            qc.append(u_A.control(1),[phase[i]]+list(xreg)+list(qubits)+list(ancilla))
    ...
    def factorize(self, counts: dict[str,int] | None = None, shots: int = 10000) -> tuple[int, int] | None:
        """
